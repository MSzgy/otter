...
修复同一 bundleId 的多个浏览器按钮都指向同一实例的问题。UI 传递 PID；自动轮询使用前台 PID；标签列表与正文目标保留 PID；IPC 验证运行实例和标签来源；多实例下旧的仅 bundleId 请求拒绝执行。原生 ScriptingBridge 以 process identifier 寻址，并在调用前核对该 PID 的 bundleId；进程已退出返回 not_running，不回退到其他实例。每个实例独立记录读取失败后的自动重试暂停状态。

40 项 Node 测试、TypeScript/Vite 构建通过。原生实测两个 Chrome 进程分别返回 0 个窗口与 2 个窗口；在正确实例中找到了通过 Chrome UI 创建的自有 localhost 测试页。首次正文调用被误报为 javascript_permission_required；读取原始异常发现是 ScriptingBridge 动态方法未通过 selector 调用导致的 TypeError。已改用 performSelector 调用，并收紧权限错误匹配，新增两项回归覆盖 selector 调用及 TypeError 不误报权限。用户允许开启正确实例的 Apple Events JavaScript 后，原生测试已成功取得自有页面固定正文，且隐藏/表单固定文字均未进入结果；此为原生链路证据，尚需新版安装包 UI 端到端验证。没有输出用户其他标签的标题、URL 或正文。

0.7.3 个人 Applications 安装副本通过严格签名校验及真实 UI 验收：选择包含测试页的 Chrome 实例后显示 2 个窗口、22 个标签页；点击自有标签正文，Otter 切到聊天页，预览准确包含测试页标题、来源、固定正文，且无隐藏/表单测试文字。测试未发送到模型，附件及自有标签已清理。用户实际标签标题和地址不保存到仓库。

升级后的 ad-hoc 签名令辅助功能检查再次显示未授权，故 0.7.2 的 AX/选区通过证据不能自动沿用为 0.7.3 已通过；需恢复同一 Otter 的授权。正式稳定升级需要 Developer ID 签名，不能使用放宽签名身份校验绕过权限。
